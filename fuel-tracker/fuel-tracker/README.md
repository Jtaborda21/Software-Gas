...
## 7. v2 upgrade: localization, i18n, onboarding, 3D visualizer

If you already have a v1 database, run
`supabase/migration_002_localization_onboarding.sql` once in the SQL
Editor — it adds the new `make`/`model`/`model_year`/`trim`/`color_hex`/
`onboarded` columns to `vehicles` and flips the defaults for *new*
vehicles to COP/gallons (it does not touch your existing rows' currency
or units; uncomment the one `update` line in that file if you want to
convert your existing vehicle too).

New packages this adds — run `npm install` after pulling:
`three`, `@react-three/fiber`, `@react-three/drei`, `@types/three`.

- **Currency**: all money is now formatted with `formatCOP()` in
  `lib/format.ts` — `Intl.NumberFormat("es-CO", { style: "currency",
  currency: "COP" })`. This is fixed regardless of the UI language toggle,
  per spec.
- **Units**: new vehicles default to `volume_unit: "gal"`; every
  efficiency/cost calculation was already parameterized off
  `vehicle.volume_unit` and `vehicle.distance_unit`, so Km/Gal falls out
  automatically — no math changed, only the default unit.
- **i18n**: `lib/i18n/LanguageContext.tsx` provides a `useTranslation()`
  hook (`t("key", { placeholders })`) backed by the dictionaries in
  `lib/i18n/translations.ts`. `LanguageSwitcher` in the header/login
  screen toggles it, saved to `localStorage`. To add a string, add the
  key to both the `en` and `es` blocks in `translations.ts`.
- **Onboarding**: `components/onboarding/OnboardingModal.tsx` shows
  automatically whenever `vehicle.onboarded === false` (i.e. right after
  signup, since the new-user trigger creates a vehicle with that default).
  Make → Model → Year → Trim are all tap-to-select from
  `lib/data/vehicleDatabase.ts` — extend that array with more vehicles;
  no component code needs to change.
- **3D visualizer**: `components/CarVisualizer3D.tsx` renders a
  procedural placeholder car (primitives, not a real model) with
  `OrbitControls` for 360° rotation/zoom and a live paint-color prop.
  `components/CarVisualizerCard.tsx` wraps it with the color-picker
  swatches and persists the chosen color back to `vehicles.color_hex`.
  To use real car models later, swap the body of `CarModel` for
  `useGLTF(\`/models/${vehicle.make}-${vehicle.model}.glb\`)` from
  `@react-three/drei` — the color/OrbitControls wiring won't need to change.
- **Auth**: the Google OAuth button has been removed from `AuthForm` for
  now — only email/password remains. The dynamic-origin fix for email
  confirmation links (`${window.location.origin}/auth/callback` on
  sign-up, and `origin` read from the incoming request in
  `app/auth/callback/route.ts`) is in place either way, so re-adding
  Google later is just restoring the `signInWithOAuth` call and button.
