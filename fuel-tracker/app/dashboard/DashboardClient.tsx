...
import { RefuelRecord, RefuelRecordInput, Vehicle } from "@/lib/types";
...
export default function DashboardClient() {
  ...
  return (
    <div className="min-h-screen pb-28">
      <header className="sticky top-0 z-30 backdrop-blur bg-dash-bg/80 border-b border-dash-line">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-gauge-amber/10 border border-gauge-amber/30 flex items-center justify-center">
              <Fuel className="w-4.5 h-4.5 text-gauge-amber" />
            </div>
            <div>
              <h1 className="font-display text-base font-semibold leading-tight">{vehicle.name}</h1>
              <p className="text-xs text-dash-muted leading-tight">{userEmail}</p>
            </div>
          </div>
          <button
      ...
      >
        <Plus className="w-5 h-5" /> Log fill-up
      </button>
